...
本执行层解决六个片段阶段（Terminologist、Translator、Editor、Translator revision、Polisher、QA）在长文本中逐片
串行导致的吞吐问题，同时保持以下不变量：
...
6. Editor 的事实性意见只为对应片创建 revision 任务；没有事实性问题的片零调用沿用初稿。
7. QA 返工只重跑不通过或发生完整性错误的片段，其他片段沿用上一轮结果。
...
LangGraph 的阶段依赖为 `terminology → translate → edit → revise → polish → qa`。并发发生在同一阶段
...
llm:
  budget:
    max_requests: 150          # 所有 tier、worker 和供应商级重试共享；0 表示不限
    max_reserved_tokens: 1500000
...
项目的已验收目标配置为 20 个在途请求；首次接入新供应商时仍建议从 4、8、12
逐级提升，并根据并发限制、429 比例、平均延迟和费用决定是否启用 20。阶段配置
可以高于全局值，但不会突破全局上限。

`concurrency.budget.max_segment_calls` 按片段 Agent 任务计数，包含 Terminologist；
达到后，未派发片段以 `BudgetExceeded` 失败结果进入现有片段级降级逻辑。全局或
对应阶段达到失败上限后熔断该范围的新任务。已经在途的请求允许完成，避免供应商侧
仍计费但本地丢失结果。

`llm.budget` 位于更靠近真实供应商的边界：每个 SDK 请求发出前原子预留一次，
fast/strong、并发 worker、HTTP 重试及流中断后的完整重试共享同一计数。
`max_reserved_tokens` 以 prompt UTF-8 字节数 + 64 消息余量 + completion
`max_tokens` 计算保守上界；超限请求不会发出。CLI 元数据的 `llm_request_budget`
记录最终请求数和预留量。无 key/无 SDK 的 `[DRAFT]` 不计为真实请求。
...
结果还必须匹配 `input_hash` 才能复用。通用 v4 指纹覆盖 Agent 类与 Prompt 模板、
角色档位、temperature、max_tokens、结构化 JSON/修复策略、供应商模型、端点、
超时/重试参数、原文、上下文、轮次和阶段。密钥值绝不进入指纹或 manifest。
revision 另叠加仅属于 revise 阶段的 v5 unit-ID 协议语义；以后只修改 revision
Prompt/校验器不会让 Terminology/Translate/Edit checkpoint 一并失效，下游仍按实际
修订稿和批注状态变化自然失效。
只有 `ok=true` 的结果会被业务恢复，失败结果只供诊断并在恢复时重新执行。

启用 manifest 后，每次完成的运行会把恢复所需状态写入
`data/runtime/runs/<run_id>.json`。仅重试 QA 技术失败、语义 rework 或缺失片：
...
若最终问题来自 Revision 协议拒绝或 QA 仍判返工，可从 Revision 定向恢复：

```powershell
mant resume-run --config config/settings.yaml `
  --run-id run-20260718-example --stage revise --failed-only `
  --stream --verbose
```

恢复时会重新读取原文件并核对规范化正文；正文变化会直接拒绝恢复。QA 恢复从
QA 节点进入，不再调用术语、翻译、审校或润色；成功 QA checkpoint 复用，失败/
rework/缺失项重跑。Revision 恢复会把最终 `rework_segment_indices`、Revision/
Polish/QA 失败以及逐片 QA rework 取并集；`--failed-only` 只选择这个集合，随后为
所选片强制执行一次 `Revise → Polish → QA`，保留其他片及其 QA 结果。该链路不会
调用 Terminology/Translate/Edit，也不会在 QA 仍失败时自动进入整段重译。
`--no-failed-only` 则选择全部片段。角色配置改变后相关指纹失效，因此会安全重跑
受影响的目标阶段结果。没有 manifest 且使用旧指纹的运行不能直接定向恢复。
...
首轮正文五个阶段处理全部片段，其中 revise 只为含事实性意见的片创建调用。QA 汇总以下序号到
`TranslationState.rework_segment_indices`：

- QA 返回 `rework` 或 QA 调用失败的片段；
- Translator、Editor、revision、Polisher 完整性检查记录失败的片段。

进入下一轮后，五个阶段只处理这些序号；revise 仍按意见类型按需调用。未选中的
`draft_segments`、`revised_segments`、`polished_segments` 和 `segment_qa` 原位
保留，最终仍按原始片段序号拼接。若章级
...
- `llm.budget` 已提供供应商请求数和保守 token 硬上限，但不是准确金额：精确账单
  仍需要供应商 tokenizer、缓存计价、stream usage 与可版本化价格表共同支持。
...
- review notes 已带稳定 `note_id`、轮次、片段、来源和 `resolution`。Translator
  重译仍标记 `translation_applied`；unit-ID 定点修订应用或 no-change 会先进入
  `revision_*_pending_qa`，该片 QA pass 后才变为 `resolved`，QA rework 则重新打开。
  协议拒绝保存为语义失败并保留原稿，不增加调度器技术失败计数。
