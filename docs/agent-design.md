...
- **Prompt 设计要点**：
  ...
  - 每片最多返回 24 个跨章节高价值专名，禁止普通名词和穷举；代码按置信度与名称
    稳定裁剪，默认输出上限 1,536 token；
...
- **Prompt 设计要点**：
  ...
  - revision mode 不重写整段。代码先按换行/句末把当前译文切成保留原始间隔的
    单元，并给出 `unit_id + expected_hash`；模型只返回引用这些 ID 的
...
| --- | --- | --- | --- | --- |
| 术语 | fast | 按片预算 | 每片 ≤1,536 | 最多 24 个高价值术语，代码侧裁剪归并 |
