...
- **职责**：章节翻译前扫描全文：① 用 `lookup_terms` 命中已有术语；② 对未命中的疑似术语（人名、地名、功法、法宝、组织、头衔）做抽取与翻译；③ `record_terms` 入库；产出本章 `glossary`。
- **一致性原则**：同一 `work_id` 内"先入库者为准"；新抽取术语与库内冲突时保留库内译名，仅当 `confidence` 显著更高（如高 0.2 以上）时记为待人工裁决，不自动覆盖。
- **输入**：`AgentTask.source_text` = 整章原文（`segment_id="chapter"`），`context.bible` 辅助消歧。
- **输出**：`glossary`（本章生效的映射）、`new_terms`（本次新入库条目）。
- **Prompt 设计要点**：
  - 输出严格 JSON 数组，元素字段固定；无新术语返回 `[]`；
...
| --- | --- | --- | --- | --- |
| 术语 | fast | 4,000 | 600 | 整章扫描，输出 JSON |
