...
def _normalize_review_note(
    ...
    *,
    ...
    return {
        ...
        "span": str(item.get("span", "")).strip()[:max_span_chars],
...
class EditorAgent(BaseAgent):
    ...
    #: 审校只输出问题列表；限制冗长分析，降低结构化角色的 completion 成本。
    max_tokens: int = 1536
    #: 正常审校最多保留的意见数；模型必须合并同类问题并优先 high。
    max_review_notes: int = 6
    #: 单条定位和建议的字符硬上限，避免合法 JSON 仍异常膨胀。
    max_span_chars: int = 80
    max_suggestion_chars: int = 160
    #: 输出截断或 schema 无效时，改用更小契约重新审校，而非原样重试。
    compact_recovery_attempts: int = 1
    compact_recovery_max_tokens: int = 768
    compact_recovery_max_notes: int = 3
...
- 合并同类或相邻问题，按 high → medium → low 排序；不得超过本次硬限制。
- span 和 suggestion 必须简短；禁止复述整段原文或译文。
...
若完全没有问题，输出：{"review_notes": []}"""
    ...
    COMPACT_RECOVERY_SYSTEM_PROMPT: str = """上一次审校输出过长或结构无效。
重新对照原文与译文，只报告最影响忠实度的少量问题；必须优先漏译、事实性误译和
专名错误，合并相邻或同类问题。只输出合法、紧凑的 JSON 对象，禁止解释、代码块、
逐段复述或额外字段。格式必须为：
{"review_notes":[{"issue_type":"omission|mistranslation|proper_noun|other",
"span":"短定位","suggestion":"短建议","severity":"high|medium|low"}]}"""
...
请逐段对照审校，并按要求只输出 JSON。"""
    ...
    def run(self, task: AgentTask) -> AgentResult:
        ...
        response_format = {"type": "json_object"} if self.structured_json else None
        system = (
            self.SYSTEM_PROMPT
            + "\n\n【本次硬限制】最多输出 "
            + str(self.max_review_notes)
            + " 条意见；span / suggestion 分别不得超过 "
            + f"{self.max_span_chars} / {self.max_suggestion_chars} 个字符。"
        )

        # 3) 调用并解析。原输出截断或 schema 无效时，使用更小契约恢复一次；
        #    不重复相同提示和预算，避免稳定超长响应再次耗尽上限。
        try:
        ...
        except Exception as exc:  # noqa: BLE001 —— Agent 边界统一降级
            notes.append(f"LLM 调用失败：{exc!r}")
            return self._result(ok=False, output={"review_notes": []}, notes=notes)
        notes.extend(getattr(self.llm, "last_notes", []))

        parsed = self.parse_json_output(raw, notes)
        review_notes = _extract_review_notes(
            parsed,
            max_span_chars=self.max_span_chars,
            max_suggestion_chars=self.max_suggestion_chars,
        )
        note_limit = self.max_review_notes
        for attempt in range(self.compact_recovery_attempts):
            if review_notes is not None:
                break
            recovery_system = (
                self.COMPACT_RECOVERY_SYSTEM_PROMPT
                + "\n最多输出 "
                + str(self.compact_recovery_max_notes)
                + " 条；span / suggestion 分别不得超过 "
                + f"{self.max_span_chars} / {self.max_suggestion_chars} 个字符。"
            )
            try:
                recovered = self.llm.complete(
                    recovery_system,
                    user,
                    temperature=0.0,
                    max_tokens=self.compact_recovery_max_tokens,
                    response_format=response_format,
                    thinking=self.thinking,
                )
            except Exception as exc:  # noqa: BLE001 —— 恢复失败仍由结果契约承接
                notes.append(f"紧凑审校恢复调用失败：{exc!r}")
                break
            notes.extend(getattr(self.llm, "last_notes", []))
            recovered_parsed = self.parse_json_output(recovered, notes)
            review_notes = _extract_review_notes(
                recovered_parsed,
                max_span_chars=self.max_span_chars,
                max_suggestion_chars=self.max_suggestion_chars,
            ...
            if review_notes is not None:
                note_limit = min(
                    self.max_review_notes, self.compact_recovery_max_notes
                )
                notes.append(f"审校输出已在第 {attempt + 1} 次紧凑恢复后恢复")

        # 4) 对合法输出再次执行确定性排序和数量裁剪，模型不能突破预算契约。
        output_valid = review_notes is not None
        if review_notes is None:
            notes.append("审校意见结构不符合 schema，降级为空意见列表（安全默认）")
            review_notes = []
        severity_order = {"high": 0, "medium": 1, "low": 2}
        review_notes.sort(key=lambda item: severity_order[item["severity"]])
        if len(review_notes) > note_limit:
            notes.append(
                f"审校意见共 {len(review_notes)} 条，已按严重度保留前 "
                f"{note_limit} 条"
            )
            review_notes = review_notes[:note_limit]

        return self._result(
            ok=output_valid, output={"review_notes": review_notes}, notes=notes
        )
